...
- File identity is path-based; re-probing is decided by dev/inode + size + mtime, and the stored sample hash (xxh3-128 over three 256 KB windows: head, middle, tail) is only a change signal for the applied-operations record (design doc §4); the in-file processed marker's fingerprint is xxh3-128 over the applied operation (§6.6). Don't invent whole-file hashing — it doesn't scale to 4K libraries.
