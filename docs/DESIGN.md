...
**Change detection / probe cache.** A file's facts are re-probed only when any of (path, dev, inode, mtime, size) changes. The stored **sample hash** — **xxHash64** over the first 8 KB and last 8 KB of the file (recomputed whenever the row changes, so a changed hash clears the applied-operations record, §6.6) — is a secondary change signal for byte-level edits; it is not the file's identity (that is path + dev/inode + size + mtime). The operation **fingerprint** of §6.6 (xxHash3-128) is a different hash with a different job: it names *which operation* produced the file's bytes and lives in the file itself, so it survives a lost database.
...
Filter graphs are **one-shot transformations**: the output's new facts do not record that a filter was applied (a cropped H.264 is still H.264), so "already done" is decided from what the file **records about itself** rather than from re-evaluation: the per-file **applied-operations record** plus, in-place, the **in-file marker** (§6.6). A rescan re-queues a plan only when the plan's fingerprint differs from the recorded one (the flow's operation changed) or when the file's bytes have changed (the new sample hash clears the record). The record is written immediately after the file swap (before the gate) so a crash in between cannot cause a double application; the byte check is only as strong as the sample hash (xxHash64 over a 16 KB head+tail sample, §4). A graph must not undo the flow's other constraints for that step (e.g. a `scale` beyond the downscale target): the post-job gate then sees a non-compliant output, the job fails `non_idempotent`, and every scan re-queues the file.
...
- **Rust**, single static binary. `tokio` for async I/O; `rusqlite` for SQLite; **ffmpeg as a child process** — no libav bindings, so a crashed/hung encoder can never take the server down, and the binary stays cgo/FFI-free. All non-cryptographic hashing is **xxHash** via the MIT-licensed `twox-hash` crate (xxh64 for the sample hash, xxh3-128 for the in-file fingerprint — §4, §6.6); the better-known `xxhash-rust` crate is BSL-1.0 and excluded by the AGPL rule.
...
15. **All non-cryptographic hashes are xxHash** (§4, §6.6): xxHash64 for the 16 KB sample hash (replacing FNV-1a 64 — pre-v1.0, a clean break; a one-time upgrade migration re-stamps stored hashes from the current bytes and preserves the applied-operations records, so an upgrade never reprocesses a file) and xxHash3-128 for the fingerprint embedded in files. Stability is enforced in CI in two directions: the hash digests are pinned to published known-answer vectors, and the *fingerprint* is pinned end-to-end (a fixture operation JSON → an exact marker string), so no release can change canonicalization and silently invalidate a library on upgrade; and a marker write→probe round-trip is asserted for every output container, so a change in the pinned ffmpeg's metadata behavior fails the build rather than quietly losing markers.
