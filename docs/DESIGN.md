...
**Status**: v0.2 — v0.1 was settled through the grilling session; v0.2 adds the processed-marker / applied-operations design (§6.6) and the all-xxHash decision (§13.15). This is the agreed design; propose changes here before coding.
...
**Change detection / probe cache.** A file's facts are re-probed only when any of (path, dev, inode, mtime, size) changes. The stored **sample hash** — **xxh3-64** over the first 256 KB and last 256 KB of the file (recomputed only when the row changes, so a changed hash clears the applied-operations record, §6.6) — is a secondary change signal for byte-level edits; it is not the file's identity (that is path + dev/inode + size + mtime). The 256 KB end-windows (up from 8 KB) target where in-place edits actually land: container metadata (MP4 `moov`, MKV cues) lives at the file's ends, so the window catches metadata rewrites that change no other fact; two sequential reads, computed only on changed files, so even large libraries stay cheap (§13.15). The operation **fingerprint** of §6.6 (xxh3-128) is a different hash with a different job: it names *which operation* produced the file's bytes and lives in the file itself, so it survives a lost database.
...
Filter graphs are **one-shot transformations**: the output's new facts do not record that a filter was applied (a cropped H.264 is still H.264), so "already done" is decided from what the file **records about itself** rather than from re-evaluation: the per-file **applied-operations record** plus, in-place, the **in-file marker** (§6.6). A rescan re-queues a plan only when the plan's fingerprint differs from the recorded one (the flow's operation changed) or when the file's bytes have changed (the new sample hash clears the record). The record is written immediately after the file swap (before the gate) so a crash in between cannot cause a double application; the byte check is only as strong as the sample hash (xxh3-64 over the 256 KB head+tail sample, §4). A graph must not undo the flow's other constraints for that step (e.g. a `scale` beyond the downscale target): the post-job gate then sees a non-compliant output, the job fails `non_idempotent`, and every scan re-queues the file.
...
- **Rust**, single static binary. `tokio` for async I/O; `rusqlite` for SQLite; **ffmpeg as a child process** — no libav bindings, so a crashed/hung encoder can never take the server down, and the binary stays cgo/FFI-free. All non-cryptographic hashing is the **xxh3 family** via the MIT-licensed `twox-hash` crate (xxh3-64 for the sample hash, xxh3-128 for the in-file fingerprint — §4, §6.6); the better-known `xxhash-rust` crate is BSL-1.0 and excluded by the AGPL rule.
...
15. **All non-cryptographic hashes are the xxh3 family** (§4, §6.6): xxh3-64 for the sample hash (256 KB head + 256 KB tail — up from FNV-1a over 16 KB; pre-v1.0, a clean break, with the one-time migration re-stamping stored values from current bytes so an upgrade never reprocesses a file) and xxh3-128 for the fingerprint embedded in files. Both widths come from one family: xxh3 is the generation upstream recommends (the classic xxh64/xxh32 are kept for compatibility), and it is the faster of the two once the window widens (SIMD paths; classic xxh64 has none). The window stays at the file's ends rather than sampling scattered offsets — the edit threat is container metadata (MP4 `moov`, MKV cues), which lives at the ends, and scattered reads cost more on NAS than the sequential bytes they save; the never-whole-file rule stands. Stability is enforced in CI in two directions: the hash digests are pinned to published known-answer vectors, and the *fingerprint* is pinned end-to-end (a fixture operation JSON → an exact marker string), so no release can change canonicalization and silently invalidate a library on upgrade; and a marker write→probe round-trip is asserted for every output container, so a change in the pinned ffmpeg's metadata behavior fails the build rather than quietly losing markers.
